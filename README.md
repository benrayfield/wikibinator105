...
* Header size of a function id is 1-64 bits and is deterministicly generated from the headers of its 2 childs and 1 byte from each of where is their last 1 bit in those 256 bits in case they are literal 256 bits thats its own id which happens when the first byte is 0xf9. Similarly, every id256 is deterministicly generated from its 2 child id256s, and is lazyEvaled, needing only the header and that "where is their last 1 bit" such as to wrap an int[234221] in a lambda without generating its id256 yet or ever.
